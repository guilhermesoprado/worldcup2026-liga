...
export class MatchRepository extends BaseRepository {
  ...
  async replacePhaseMatches(phase: string, matches: ReplaceMatchInput[]) {
    const db = this.ensureDb();

    const { error: deleteError } = await db.from("matches").delete().eq("phase", phase);
    if (deleteError) throw deleteError;
